...
(Recall from \S\ref{intro.sentences} that we're only considering propositional sentences, at least for now.)
...
A \define{regimentation} (\define{symbolization}) of an argument in English is an argument in SL whose sentences regiment the sentences of the argument in English.
...
This is regimentation of the argument, but it's not a very interesting one.
...
We use capital Roman alphabet letters for the sentence letters of SL.\footnote{Later on, when we come to the logic of for-all and some, we will reserve \textit{lowercase} Roman alphabet letters for constants and variables. Hence, there is a \textit{good} reason why you will be forced to capitalize on \textit{Carnap}.} 
...
The question of which logic to accept for which applications is a deep and contentious issue within philosophical logic.
...
Although competing logics (notably intuitionistic logic) may claim to hold certain philosophical advantages, not all proofs of mathematics can be reconstructed in their terms.
...
For instance, the modal logics that you would learn about in an intermediate logic course have also been shown to have powerful applications within semantics, computer science, and philosophy, and may be naturally combined with the logics with which we will be concerned.
Rather than any kind of stopping place, the logics covered in this course make for a natural place to begin.
...
Despite the advantages afforded by the classical logics we will be concerned with, these logics will be rather artificial by comparison to the informal patterns of reasoning in English with which you are already familiar.
...
Once we add a new atomic sentence $M$ for `Matt is strong', this translates to $J\eand M$.\footnote{One might contest this claim by instead taking `Jessica and Matt' to name a plurality where `are strong' is a plural predicate. We will be overlooking such complexities which are better handled in plural logics.}
