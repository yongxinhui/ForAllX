...
\begin{earg}
  \eitem{Edinburgh is North of Oxord.}
  \uitem{Oxford is North of London.}
  \eitem{Edinburgh is Nort of London.}
\end{earg}
...
In addition to providing a means by which to resolve the ambiguities of natural language, regimenting English arguments in formal languages may be used to provide an account of their logical form.
...
\textbf{Exclamations} Expressions like `Ouch!' or `Boo, Yankees!' are sometimes described as exclamatory sentences, but they are neither true nor false.
...
In contrast to the types of sentences above, declarative sentences describe \textit{ways for some particular things to be} which we will refer to as \define{propositions}.
...
The \define{truth-value} of a declarative sentence on an interpretation is either the value `True' or `False' depending on whether the proposition that sentence expresses on that interpretation happens to obtain.
...
More specifically, we will \define{interpret} the sentences of our various languages by assigning them truth-values, ignoring any further differences that this may neglect.
% may occur between the propositions that those sentences may express on an interpretation
The present investigation belongs to \define{classical logic} insofar as we will require interpretations to assign sentences to either true or false and never both at once.
...
In both of the arguments above, every possibility in which premises are true is one in which the conclusion is true, and so the conclusion follows as a matter of \define{necessity} from the premises.
Put otherwise, the premises \define{strictly imply} the conclusion in each of the arguments above.
...
\begin{earg}
  \eitem{Socrates is human.}
  \uitem{Every human is mortal.}
...
What it is for a sentence to be a \define{logical consequence} of a set of sentences is for the former to be true on any interpretation in which every sentence in the latter is true. 
...
Since, given the intended interpretation, it has a false premise--- premise \eref{ch00:fruit}{2}--- it does not establish its conclusion (it is not a sound argument), but it does have a valid \emph{logical form}.
...
Since `Socrates is human' is a sentence and doesn't have any parts that are also sentences, it is an \define{atomic sentence}, and so all we can do is replace it with a variable.
Let's choose `$\metaA$'. 
...
Sentence O1 is true on some interpretations and false on others and so is said to be \define{logically contingent} or just \define{contingent} for short.
...
Although one may well argue form \eref{ch00:north}{1} to \eref{ch00:north}{2} or \textit{vice versa}, these arguments do not hold by virtue of logic alone, but rather by virtue of the intended interpretation of `is North of' and `is South of'.
