...
Given a model $\M$ that satisfies the premises $\Gamma$, this proof makes use of \textbf{\ref{lemma:model}} in order to introduce a model variant $\M'$ which assigns the constant $\beta$ to whatever the variable $\alpha$ had been assigned to by the resulting variable assignment.
...
Since $\beta$ does not occur in any premises, we may draw $\metaB$ as a consequince. 
