...
Assuming that $\beta$ is free for $\alpha$ in ever line of a proof $X$, it will ease the following exposition to take $X\unisub{\beta}{\alpha}$ to be the result of replacing $\beta$ with $\alpha$ in every line of $X$.
...
\begin{quote} 
  ...
  The proof goes by induction on the length of $X$.

  \textit{Base:} Assume that 
\end{quote}
...
Given that $\Gamma$ is consistent in QL$^=$, we may conclude that $\Gamma$ is consistent in QL$^=_{\N}$.
...
\begin{Lthm} \label{lemma:prcut}
  If $\Lambda \proves \metaA$ and $\Pi\cup{\metaA} \proves \metaB$, then $\Lambda\cup\Pi \proves \metaB$. 
\end{Lthm}
...
In order to show that $\Delta_{\Sigma_\Gamma}$ is deductively closed, consider the following general lemmas. 
...
Having extended the consistent set of sentences $\Gamma$ in QL$^=$ to a saturated maximal consistent set of sentences $\Delta_{\Sigma_\Gamma}$ in QL$^=_{\N}$ which is deductively closed, we may now proceed to use $\Delta_{\Sigma_\Gamma}$ to construct a Henkin model that satisfies $\Delta_{\Sigma_\Gamma}$, and so thereby satisfies $\Gamma$.
For ease of exposition, we will often drop the subscripts, though `$\Delta$' is to be understood as standing for $\Delta_{\Sigma_\Gamma}$.
...
Consider the following:
  ...
  \end{enumerate}
The equivalence class $[\alpha]_\Delta$ is the set which includes every constant $\beta$ in $\C$ where the sentence $\alpha=\beta$ belongs to $\Delta$.
In order to show that $[\alpha]_\Delta\neq\varnothing$ for any constant $\alpha\in\C$, we begin by proving the following weakening principle:
...
\begin{quote} 
  ...
  Together with \textbf{\ref{lemma:define}}, we may conclude that $(\star)$ holds where the other biconditionals follow from the definitions and the assumption that $\alpha_1,\alpha_2\in\C$.
...
It follows immediately from \textbf{\ref{lemma:truth}} that $\M_\Delta$ satisfies $\Delta$ and so $\M_\Delta$ satisfies $\Gamma$ since $\Gamma\subseteq\Delta$. 
Nevertheless, since $\M_\Delta$ is a model of QL$^=_{\N}$ and not QL$^=$, it remains to show that $\Gamma$ is satisfied by a model of QL$^=$.
Accordingly, we will restrict $\M_\Delta$ to QL$^=$ as follows: 
  \begin{enumerate}[leftmargin=1.5in]
...
So long as we are only interpreting constants and predicates that do belong to QL$^=$, the domain may contain anything whatsoever.
Thus we may take $\D'_\Delta=\D_\Delta$ as before, letting $\M'_\Delta=\tuple{\D'_\Delta,\I'_\Delta}$ be the restriction of $\M$ to QL$^=$. 
It is easy to show that $\M'_\Delta$ and $\M_\Delta$ satisfy the same QL$^=$ sentences, and so $\M'_\Delta$ satisfies $\Gamma$. 
