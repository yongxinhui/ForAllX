...
\factoidbox{
  A \define{derivation} (or \define{proof}) of $\metaA$ from $\Gamma$ in PL is any finite sequence of wfs of $\PL$ ending in $\metaA$ where every wfs in the sequence is either: (1) a premise in $\Gamma$; (2) an assumption which is eventually discharged; or (3) follows from previous lines by a natural deduction rule for PL besides AS. 
}

A wfs $\metaA$ of $\PL$ is \define{derivable} (or \define{provable}) from $\Gamma$ in PL, i.e., $\Gamma \vdash \metaA$, just in case there is a natural deduction derivation (proof) of $\metaA$ from $\Gamma$ in PL. 
% Whereas `$\Gamma \vdash_{T} \metaA$' was shorthand for `$\Gamma, \enot \metaA \vdash_{T} \bot$', 
...
Two sentences $\metaA$ and $\metaB$ of $\PL$ are \define{interderivable} (or \define{provably equivalent}) in PL--- i.e., $\metaA \dashv\vdash \metaB$--- just in case both $\metaA\vdash\metaB$ and $\metaB\vdash\metaA$.
% Additionally, we will take $\Gamma\vdash\bot$ to mean that $\Gamma\vdash\metaB\eand\enot\metaB$ for some $\PL$ sentence $\metaB$.
...
Letting \define{bottom} (also called the \define{falsum}) be the arbitrary contradiction $\bot \colonequals A \wedge \enot A$, we may take a set of sentences $\Gamma$ to be \define{inconsistent} just in case $\Gamma \vdash \bot$, and \define{consistent} otherwise.
...
% % For instance, it is important to disambiguate the turnstiles in making the following observation: whereas $\Gamma \vdash_{T} \metaA$ was defined as $\Gamma, \enot \metaA \vdash_{T} \bot$ for the tree proof system, here we have defined $\Gamma \vdash_{PL} \bot$ in terms of $\Gamma \vdash_{PL} \metaB\eand\enot\metaB$ for some $\metaB$. 
% So $\Gamma\vdash\metaA$ can be read as: $\metaA$ is derivable from $\Gamma$.
