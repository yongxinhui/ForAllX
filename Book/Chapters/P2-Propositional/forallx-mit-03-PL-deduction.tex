...
As with all of our methods, writing formal proofs with PL has its range of natural applications where sometimes it is more trouble than it is worth and a semantic proof would have been better.
...
This concludes our discussion of the rules of PL.
...
\factoidbox{
  A natural deduction \define{proof} (or \define{derivation}) of a conclusion $\metaA$ from a set of premises $\Gamma$ in PL is any sequence of lines ending with $\metaA$ on a live line where every line in the sequence is either: (1) a premise in $\Gamma$; (2) a discharged assumption; or (3) follows from previous lines by one of the natural deduction rules for PL. 
}

An $\PL$ sentence $\metaA$ is \define{provable} (or \define{derivable}) from $\Gamma$ in PL just in case there is a natural deduction proof (derivation) of $\metaA$ from $\Gamma$ in PL, where we may write this: $\Gamma \proves \metaA$. 
...
% Additionally, we will take $\Gamma\proves\bot$ to mean that $\Gamma\proves\metaB\eand\enot\metaB$ for some $\PL$ sentence $\metaB$.
...
We will refer to an $\PL$ sentence $\metaA$ as a \define{theorem} of PL just in case $\proves\metaA$, i.e., $\varnothing\proves\metaA$.
...
% In particular, we were concerned with interpretations which assigned the sentence letters of $\PL$ to exactly one truth-value.
% Since this is the only definition of an interpretation that we will consider for $\PL$, there is only one corresponding notion of entailment $\models$.
...
% A natural strategy for establishing this connection is to show that $\Gamma \proves_{PL} \metaA$ if and only if $\Gamma \proves_{T} \metaA$.
...
\factoidbox{
  PL is both sound and complete, i.e., $\Gamma \proves_{PL} \metaA$ if and only if $\Gamma \models \metaA$.
}
...
Given that PL is sound and complete, the \emph{theorems} of PL are coextensive with the \emph{tautologies} of $\PL$.
