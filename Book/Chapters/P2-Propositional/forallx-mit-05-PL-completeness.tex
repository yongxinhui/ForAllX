...
We begin by defining a set $\MetaD$ of wfss of $\PL$ to be \define{deductively closed} in PL just in case for any wfs $\metaA$ of $\PL$, if $\MetaD\vdash\metaA$, then $\metaA\in\MetaD$.
...
Given these results, we are now ready to construct the Henkin interpretation of $\PL$.
...
Since this construction was introduced by Leon Henkin (1949), we will refer to $\M_\MetaD$ as the \define{henkin interpretation} of $\PL$ for $\MetaG$ (recall that $\MetaD=\MetaD_{\MetaG}$).
