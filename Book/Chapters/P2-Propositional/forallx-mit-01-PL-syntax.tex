...
% TODO: move to next chapter or keep here or neither?
...
Notice that we make no attempt to provide a distinct symbol for `also' as it occurs in sentence G3.
...
Sentence H3 is a bit more complicated.
...
% TODO: move to next chapter or keep here or neither
For any two sentences $\metaA$ and $\metaB$ of $\PL$, the conjunction $\metaA\eand\metaB$ is true if and only if the conjuncts--- $\metaA$ and $\metaB$--- are both true. We can summarize this in the truth table for conjunction:
\begin{center}
\begin{tabular}{c|c|c}
...
\begin{earg}
  \eitem{Denison will golf with me or he will watch movies.}
  \eitem{Either Denison or Ellery will golf with me. }
\end{earg}
...
To fully regiment J1, we will introduce the \define{disjunction} symbol `$\eor$' where the sentences to its left and right are called \define{disjuncts}.
...
Both M1 and M2 seem clearly false.
...
\subsection{The Material Biconditional}
...
% Accordingly, we could regiment N3 as $(T \eif S)\eand(S\eif T)$.
Although we could regiment N3 by conjoining two conditional claims, we will introduce the \define{material biconditional} symbol `$\eiff$' (or \define{biconditional} for short) for this purpose.
...
We can paraphrase sentence O1 as `Unless $J$, $D$.'
...
We find a related mistake in P2 which says that the woman Kamala Harris is composed of letters which is also false. % despite the fact that P2 true. 
...
The connective that you look to first in decomposing a sentence is called the \define{main operator} of that sentence.
...
Since subscripts are rarely required, we can avoid quite a bit of trouble by simply using different capital letters. % , suppressing the subscript altogether
...
\begin{earg}
  \eitem{Alice, Bob, and Candice all went to the party.}
  \eitem{Either Alice, Bob, and Candice went to the party.}
\end{earg}
...
% TODO: transition to corner quotes
...
It is important to stress that, like the metavariables, the symbol `$\therefore$' will not be a part of the object language $\PL$, but rather a part of the metalanguage.
