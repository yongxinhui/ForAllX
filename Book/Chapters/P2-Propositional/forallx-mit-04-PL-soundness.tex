...
%
% \begin{quote} 
%   \textit{Proof:} Assume $\MetaG \vDash \metaA$ and $\MetaG \supseteq \MetaG'$.
...
\begin{Lthm} \label{lemma:PL-live}
...
\begin{Lthm} \label{lemma:PL-unsatent}
  $\MetaG \cup \set{\metaA}$ is unsatisfiable just in case $\MetaG \vDash \enot\metaA$.
\end{Lthm}
\vspace{-.2in}
...
\begin{Rthm} \label{rule:PL-DisE}
  \textbf{($\boldsymbol\vee$E)}~~ $\MetaG_{n+1} \vDash \metaA_{n+1}$ if $\metaA_{n+1}$ follows from $\MetaG_{n+1}$ by the rule $\eor$E. 
\end{Rthm}
}
...
\begin{Lthm} \label{lemma:PL-conditional}
  If $\MetaG \cup \set{\metaA} \vDash \metaB$, then $\MetaG \vDash \metaA \supset \metaB$.
\end{Lthm}
\vspace{-.2in}
...
\begin{quote} 
  ...
  Since $\V{\I}(\metaA \eif \metaB) = 1$ in both cases, $\MetaG \vDash \metaA \supset \metaB$ follows by generalizing on $\I$. 
  \qed
\end{quote}
