...
\begin{multicols}{2}
  ...
  \begin{earg}
    \eitem{Only Cara loves Pedro.}
    \uitem{DJ Faro loves Pedro.\quad\quad}
    \eitem{DJ Faro is Cara.}
  \end{earg}

  \begin{earg}
    \eitem{$\qt{\forall}{x}(Lxp \eiff x=c)$}
    \uitem{$Ldp$\quad\quad}
    \eitem{$d=c$}
  \end{earg}
...
% For instance, we may consider a set of sentence $\MetaG=\set{}$
...
Given that Leibniz's Law is valid over the semantics for $\FI$, restricting Leibniz's Law requires significant revisions to the present semantics.
