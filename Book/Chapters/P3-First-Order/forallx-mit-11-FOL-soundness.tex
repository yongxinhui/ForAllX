...
% \factoidbox{
%   \textsc{completeness:} $\MetaG \vdash_{\textsc{fol}^=} \metaA$ if $\MetaG \models \metaA$.
% }
% In the opposite direction, we show that $\MetaG \models \metaA$ is enough to conclude that $\MetaG \vdash \metaA$ by appealing to completeness.
...
Whereas attempting to prove that $\MetaG \vdash_{\textsc{sd}} \metaA$ was not guaranteed to lead to a proof or to the conclusion that no proof exists, the tree method for PL always returned an answer. 
