...
Much will be as before where instead of beginning with $\MetaG\models\metaA$ as an assumption and arguing to the conclusion $\MetaG\proves\metaA$, we will focus on establishing a closely related result: 
...
% TODO: introduction
  ...
  % basic outline of the proof
    ...
    % saturate \MetaG by adding witness wffs 
    % expand \MetaS to the maximal \MetaD
    % show that \MetaD is consistent and deductively closed
    % build Henkin model out of \MetaD
    % restrict to original language
    % show that the Henkin model satisfies \MetaD
...
These properties will enable us to show that $\MetaD$ is \define{deductively closed} insofar as $\metaA\in\MetaD$ whenever $\MetaD\proves\metaA$. 
...
%   If $\MetaG\proves\metaA$ and $\beta$ is a constant that does not occur in any $\metaB\in\MetaG$, then there is a variable $\alpha$ which does not occur in $\metaA$ such that $\MetaG\proves \qt{\forall}{\alpha}\metaC$ where $\metaC=\metaA\unisub{\alpha}{\beta}$ and the derivation of $\qt{\forall}{\alpha}\metaC$ from $\MetaG$ does not include $\beta$.
% \end{Lthm}
%
% \begin{quote} 
%   \textit{Proof:} Assume $\MetaG\proves\metaA$ where $\beta$ is a constant that does not occur in any $\metaB\in\MetaG$.
...
\begin{quote} 
  ...
  %   We then aim to show that $\MetaG_i\models\metaA_i$ for ever line $i$ in $X$. 
  %
...
% TODO: is \metaA(\alpha) in the extension or not?
...
In order to extend $\MetaG$ to a saturated set $\MetaS$, fix an enumeration $\metaA_1(\alpha_1),\metaA_2(\alpha_2),\metaA_3(\alpha_3),\ldots$ of all wffs of $\FIN$ with one free variable. %, and let $n_0$ be the first natural number that does not occur in $\metaA_0(\alpha_0)$.
...
%   $\MetaG\proves \qt{\forall}{\alpha}\metaA$ if $\MetaG\proves\metaA\unisub{\beta}{\alpha}$ where $\beta$ is a constant not occurring in $\MetaG$ or $\metaA$.
% \end{Lthm}
%
% \begin{quote} 
%   \textit{Proof:} Assume $\MetaG\proves\metaA\unisub{\beta}{\alpha}$ where $\beta$ is a constant not occurring in $\MetaG$ or $\metaA$. 
% \end{quote}
%
%
%
...
\begin{quote} 
  ...
  % TODO: would be good to discuss the cut rule above which gets used here; ditto other structural rules
...
In the proof given above we made explicit appeal to \textbf{\ref{lemma:prcut}}.
...
\begin{quote} 
  ...
  % It follows that $\MetaD_n\proves\enot\enot\metaB_n$ by \textbf{\ref{lemma:incon}}, and so $\MetaD_n\proves\metaB_n$ by DN.
  % Since $\MetaD_n\cup\set{\metaB_n}$ is also not consistent, $\MetaD_n\cup\set{\metaB_n} \proves A\eand\enot A$.
  % By \textbf{\ref{lemma:prcut}}, $\MetaD_n \proves A\eand\enot A$, and so $\MetaD_n$ is not consistent, contradicting the hypothesis. 
...
Put formally, $\MetaD$ is \define{deductively closed} just in case $\metaA\in\MetaD$ whenever $\MetaD\proves\metaA$.
...
\begin{quote} 
  \textit{Proof:} 
  Identical to \textbf{\ref{lemma:PL-prweak}}.
  % Assuming that $\Lambda\proves\metaA$, there is a proof $X$ of $\metaA$ from $\Lambda$ in FOL$^=$.
  % Since $\Lambda\subseteq\Lambda\cup\Pi$, it follows that $X$ is a proof of $\metaA$ from $\Lambda\cup\Pi$ in FOL$^=$, and so $\Lambda\cup\Pi\proves\metaA$. 
  \qed
\end{quote}
...
Given that $\M_\MetaD$ satisfies $\MetaG$ where every $\metaA\in\MetaG$ is a sentence of $\FI$, it follows from \textbf{\ref{lemma:satrest}} that $\M'_\MetaD$ satisfies $\MetaG$ where $\M'_\MetaD$ is a $\FI$ model. 
