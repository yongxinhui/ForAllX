...
\begin{quote} 
  ...
  % It follows that $\Lambda\vdash \enot\metaA$ and $\Lambda\vdash\enot\enot\metaA$ by \textbf{\ref{lemma:incon}}, and so there is some proof $X$ of $\enot\metaA$ from $\Lambda$, and some proof $Y$ of $\enot\enot\metaA$ from $\Lambda$. 
...
\begin{quote} 
  ...
  % It follows that $\MetaD_n\vdash\enot\enot\metaB_n$ by \textbf{\ref{lemma:incon}}, and so $\MetaD_n\vdash\metaB_n$ by DN.
  % Since $\MetaD_n\cup\set{\metaB_n}$ is also not consistent, $\MetaD_n\cup\set{\metaB_n} \vdash A\eand\enot A$.
  % By \textbf{\ref{lemma:prcut}}, $\MetaD_n \vdash A\eand\enot A$, and so $\MetaD_n$ is not consistent, contradicting the hypothesis. 
