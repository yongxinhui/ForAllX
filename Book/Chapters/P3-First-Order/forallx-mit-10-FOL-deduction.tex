...
\factoidbox{
  A natural deduction \define{proof} (or \define{derivation}) of a conclusion $\metaA$ from a set of premises $\MetaG$ in FOL$^=$ is any sequence of lines ending with $\metaA$ on a live line where every line in the sequence is either: (1) a premise in $\MetaG$; (2) a discharged assumption; or (3) follows from previous lines by one of the natural deduction rules for FOL$^=$. 
}

A $\FI$ sentence $\metaA$ is \define{provable} (or \define{derivable}) from $\MetaG$ in FOL$^=$ just in case there is a natural deduction proof (derivation) of $\metaA$ from $\MetaG$ in FOL$^=$, where we may write this $\MetaG \proves_{\textsc{qd}} \metaA$, or just $\MetaG \proves \metaA$ when it is clear from context which proof system we intend. 

In keeping with the definitions given before, we will refer to a $\FI$ sentence $\metaA$ as a \define{theorem} of FOL$^=$ just in case $\proves\metaA$, i.e., $\varnothing\proves\metaA$.
...
Two sentences $\metaA$ and $\metaB$ are \define{provably equivalent} (or \define{interderivable}) in FOL$^=$ if and only if both $\metaA\proves\metaB$ and $\metaB\proves\metaA$.
A set of sentences $\MetaG$ is \define{provably inconsistent} if and only if $\MetaG\proves\bot$ where $\bot$ is our arbitrarily chosen contradiction, e.g., $A\eand\enot A$.
...
\factoidbox{
  \textsc{soundness:} $\MetaG \proves_{\textsc{qd}} \metaA$ only if $\MetaG \models \metaA$.
}\vspace{-.4in}
\factoidbox{
  \textsc{completeness:} $\MetaG \proves_{\textsc{qd}} \metaA$ if $\MetaG \models \metaA$.
}
...
Recall the contrast between our two proof systems for PL.
...
Put otherwise, logical entailment is \define{decidable} in PL insofar as there is an effective procedure for determining in a finite number of steps whether $\MetaG \models \metaA$ or not given soundness and completeness.
By contrast, logical entailment in $\FI$ is \define{undecidable} insofar as there is no such method for determining whether or not $\MetaG \models \metaA$.
