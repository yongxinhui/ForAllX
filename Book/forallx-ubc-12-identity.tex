...
In order to characterize quantities in a more general way, it can be useful to introduce some abbreviations for what we will refer to as the \define{quantity quantifiers}.
...
Roughly speaking, you can take `$\beta$ is free for $\alpha$' to mean `$\alpha$ is \textit{replaceable with} $\beta$ without leading to extra binding'.

We may then take $\metaA\substitute{\alpha}{\beta}$ to be the result of replacing all free occurrences of $\alpha$ in $\metaA$ with $\beta$ where $\beta$ is required to be free for $\alpha$ in $\metaA$. 
For instance, $\forall yFxy[z/x]$ is the wff $\forall yFzy$, and $\forall yFxy[y/x]$ is undefined since $y$ is not free for $x$.
...
\begin{align*}
  ...
  % \qt{\exists_{\geq 2}}{\alpha}\metaA &\colonequals \qt{\exists}{\alpha}(\metaA \eand \qt{\exists}{\beta}(\alpha\neq\beta \eand \metaA\substitute{\alpha}{\beta})) \text{ where } \beta \text{ is free for } \alpha\\
...
Here $\coloneq$ represents that the left side is merely an abbreviation for the right side. 
...
\begin{earg}
  ...
  \item[\ex{Q4}] $\qt{\forall}{x}\qt{\forall}{y}\qt{\forall}{z}(Cmz \eif (z= x \eor z= y))$.
\end{earg}
...
\begin{earg}
  ...
  \item[\ex{Q6}] $\qt{\forall}{x}\qt{\forall}{y}\qt{\forall}{z}\qt{\forall}{w}(Cmw \eif (w = x \eor w = y \eor w = z))$.
\end{earg}
...
Although we could define this recursively in a similar fashion to what was given above, we may avoid doing so by adopting the following convention for each value of $n$.
...
Although this is one way to define cardinality quantifiers, there is another much more elegant way to do so.
...
More generally, we may introduce the following cardinality quantifiers:
