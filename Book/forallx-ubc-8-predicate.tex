...
In QL, we will take lower-case letters `$a$', `$b$', `$c$',\ldots from the beginning of the alphabet with optional subscripts to be names, referring to these terms as \define{constants}. 
...
It is for this reason that we have included infinitely many constants (given the subscripts) so that we don't run out of names.

There are other ways to refer to individuals, for instance by description.
Whereas it is indeterminate to say `the person standing next to the podium' if more than one person is standing next to the podium, this expression succeeds in picking out a unique individual if there is just one person standing next to the podium.
...
Although definite descriptions which succeed in referring to unique objects are singular terms, we will not need to include primitive terms in our language to express definite descriptions.
...
In QL, we will use lower-case letters `$x$', `$y$', `$z$',\ldots from the end of the alphabet with optional subscripts to be variables.
...
\textsc{One-place} predicates are used to express properties of individuals.
...
This corresponds to the idea that the predicate expresses a property that a single object may instantiate or fail to instantiate.
For instance, either Kate is hungry or she isn't.
Accordingly, the sentence `Kate is hungry' is true or false, but the truth-value of this sentence only depends on Kate and whether she is hungry or not.

\textsc{Two-place} predicates are used to express relations between individuals.
For instance, `is taller than' is a two-place predicate which expresses a height relation between objects.
Given a two-place predicate together with two singular terms, we may construct an atomic sentence which is either true or false.
For instance, `Sam is taller than John' is true just in case Sam is taller than John.
It is possible to use a two place predicate together with two instances of the same singular term.
Just as we may say that `Jay knows Cary', we may say, `Jay knows Jay', or somewhat more naturally, `Jay knows himself'.
Although there is just one object at issue in such cases, the predicate `knows' is two-place all the same. 

The singular terms that a predicate requires to complete a sentence are referred to as the \define{arguments} of that predicate.
Whereas one-place predicates have one argument and two-place predicates have two arguments, in general we may consider $n$-place predicates which have $n$ arguments. 
...
In order to symbolize predicates in QL, we will use capital letters $A$ through $Z$, with or without subscripts.
In order to indicate both the arity and interpretation of these predicates, symbolization keys will use variables.
...
Since monadic predicates only have one argument, the variable doesn't do much work at all.
...
In order to put these predicates to work, we may now turn to combine the primitive elements that we have included in QL so far.
...
Since the `$x$' in the symbolization key entry for `$Ax$' is just a placeholder, we have replaced the variable with the constant which names Donald, resulting in what we will refer to as an \define{atomic sentence}.

Whereas `$Ad$' includes the monadic predicate `$A$', not all atomic sentences are like this.
For instance, sentence \ref{terms2} can be regimented with the dyadic predicate `$S$' as: $Sdg$.
In general, atomic sentences will include some $n$-place predicate followed by $n$ singular terms. 
In the case above, the singular terms are all constants, but often these may also include variables.
...
Nevertheless, we must begin by constructing atomic sentences in order to regiment this sentence.
To begin with, we may identify its atomic parts, regimenting these as: $Ad$, $Ag$, and $Am$.
If you like, you can think of these atomic sentences as replacing the need to introduce sentence letters as we did in SL.
Otherwise, much is as before, where we may regiment the whole by: $Ad \eif (Ag \eand Am)$.
...
In particular, we may regiment sentence \ref{open3} by first identifying the atomic sentences.
To do so, we may observe that the object in question is said to be one of two colors, and that the person in question likes both of those colors.
...
It is worth considering a closely related sentence:
...
Thus we have the atomics: $Hxc$, $Hxr$, $Lyc$, and $Lyr$.
We can then build the following regimentation: $(Hxc \eor Hxr) \eand (Lyc \eand Lyr)$.
Despite how similar the sentences \ref{open1} and \ref{open2} are to each other, their regimentations are very different, and by no means the only regimentations that we might give.
...
As before, we may build the atomic sentences $Hx$, $Mx$, and $\enot Eyx$, drawing on these to construct: $Hx \eif (Mx \eand \enot Eyx)$.
By itself, this open sentence appears to refer to some unnamed individual, however, this hardly its power.
Rather, it is by combining open sentences with quantifiers that we may make general claims of considerable interest and power.
...
Although it may not be clear exactly who, the sentence `It is happy' is presumable about whoever `It' refers to where this is some individual.
By contrast, claiming that someone is happy does not appear to be about any particular individual at all.
For instance, suppose the auditorium is filled with happy people but `It' happens to be used to refer to one of the tables.
Although sentence \ref{qu1} may then be true, `It is happy' is false.

Instead of asserting anything about a particular, quantifiers make claims about a range of individuals which we will refer to as the \define{domain}.
...
These are important differences, and we will have a lot more to say about them in the following chapter where we will introduce a semantics for QL.

Although the `It is happy' does not say the same thing as sentence \ref{qu1}, we may nevertheless begin by regimenting `It is happy' in order to regiment sentence \ref{qu1}.
Given the symbolization key above, this yields: $Hx$.
What is missing is to add that \textit{there is something in the domain} where it is happy.
We will add this piece with an \define{existential quantifier} `$\exists x$', where this yields: $\exists x Hx$.
% Here the quantifier `$\exists x$' \define{binds} all instances of the variable `$x$' in the open sentence `$Hx$'.
Whereas `$x$' is \define{unbound} in the open sentence `$Hx$', the variable `$x$' is \define{bound} in `$\exists x Hx$'.
Since there are no unbound variables in `$\exists x Hx$', this sentence is \define{closed}.
...
However cumbersome, this reading makes the atomic sentence plain to see: $Gxd$.
Although `$Gxd$' contains the constant `$d$', it also contains the unbound variable `$x$' which we may bind with an existential quantifier as before: $\exists x Gxd$.
We may then negate the result: $\enot \exists x Gxd$.
...
Note that this sentence is open since it includes the unbound variable $x$. 
Finally, we add the \define{universal quantifier} `$\forall x$', where this yields: $\forall x \enot Gxd$.
It turns out that this reading is logically equivalent to the first regimentation of sentence \ref{qu2} considered above, however, in many cases sentences will admit of multiple regimentations which are not all logically equivalent.
...
On its own, `$\exists x$' may be read `there is something such that', and similarly `$\forall x$' may be read `everything is such that'.
...
In general, $\forall x \metaA$ is logically equivalent to $\enot\exists x\enot\metaA$, and similarly, $\exists x \metaA$ is logically equivalent to $\enot\forall x\enot\metaA$.
...
Notice that without the help of variables, it is easy to lose track of which quantifier is binding to which variable.
...
Often the best answer is to proceed in stages.

We may begin by taking sentence \ref{qu3} to say that something is such that it loves somebody.
Thus we have: $\forall x LS x$ where we have temporarily introduced `$LSx$' for `$x$ loves somebody'.
Next we may take `$LS x$' to mean `there is some $y$ where $x$ loves $y$' which we may regiment: $\exists y Lxy$.
By replacing `$LS x$' in our first pass regimentation with `$\exists y Lxy$' yields the final product: $\forall x \exists y Lxy$.
In general, sentences with mixed quantifiers are tricky and require a lot of care to get right.
Even if some case you can immediately intuit a correct regimentation, it is important to learn how to systematically decompose complex cases into simple parts before putting the pieces back together.
